...
// 处理选中的JPG文件，复制标签和注释到对应的RAW文件
async function processSelectedJpgs(selectedJpgs, allItems, rawExts) {
    ...
    return { processedCount, itemsToSave };
}

// 处理选中的RAW文件（标签为空时从JPG复制）
async function processSelectedRaws(selected, allItems, rawExts) {
    ...
    return { rawProcessedCount, itemsToSave };
}
...
async function copyTagsFromJPGtoRAW() {
    try {
        ...
        // 只有当有JPG或RAW文件时才继续处理
        if (selectedJpgs.length === 0 && selectedRaws.length === 0) {
            log('没有选中的JPG或RAW文件', 'info');
            return;
        }
        ...
        if (selectedJpgs.length > 0) {
            ...
            let { processedCount, itemsToSave } = await processSelectedJpgs(selectedJpgs, allItems, rawExts);
            allItemsToSave = allItemsToSave.concat(itemsToSave);
            totalProcessed += processedCount;
        }
        
        // 处理选中的RAW文件
        if (selectedRaws.length > 0) {
            let { rawProcessedCount, itemsToSave } = await processSelectedRaws(selected, allItems, rawExts);
            allItemsToSave = allItemsToSave.concat(itemsToSave);
            totalProcessed += rawProcessedCount;
        }
        
        // 统一保存所有修改过的项目
        if (allItemsToSave.length > 0) {
            log(`开始保存 ${allItemsToSave.length} 个文件的更改...`, 'info');
            for (let i = 0; i < allItemsToSave.length; i++) {
                await allItemsToSave[i].save();
                log(`已保存第 ${i + 1}/${allItemsToSave.length} 个文件`, 'info');
            }
        ...
        let summary = `处理完成！`;
        if (selectedJpgs.length > 0) {
            summary += ` 处理了 ${selectedJpgs.length} 个JPG文件`;
        }
        if (selectedRaws.length > 0) {
            summary += ` 处理了 ${selectedRaws.length} 个RAW文件`;
        }
        summary += `，成功复制 ${totalProcessed} 个文件的标签和注释`;
        log(summary, 'success');
